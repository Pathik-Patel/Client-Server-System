...
#include <sys/wait.h>
//The program will use port number 9000 for network communication.
#define PORT 9000
// server will be able to handle up to 6 clients.
...
char* findfile(char* filename) {
    ...
    //return the string containg information.
...
int main(int argc, char *argv[]) {
    ...
    // sets socket options using the setsockopt() function.
    if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &optval, sizeof(optval))) {
       // If setting the socket options fails, an error message is printed
    ...
    while (1) {
        ...
        //client is connected to mirror
        printf("Client connected to mirror-------\n");
        //A child process is created to handle the client's requests
        childpid = fork();
        //if process id of child is less than 0
        if (childpid < 0) {
            //server-mirror  fork error.
            perror("TCP Server Mirror - Fork Error");
            exit(EXIT_FAILURE);
        }

        if (childpid == 0) {
            //The server socket is closed in the child process 
            close(server_fd);
